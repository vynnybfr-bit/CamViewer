...
class MainActivity : Activity() {
    ...
    private fun showMultiView(multiView: MultiView) {
        ...
        val selected = multiView.cameras.take(2).mapNotNull { name ->
            cameras.firstOrNull { it.name == name }
        }
        if (selected.size != 2) {
    ...
    private fun showSettings() {
        ...
        val password = editText("Senha").apply {
        ...
        content.addView(makeButton("Adicionar câmera") {
            val cameraName = name.text.toString().trim()
            val cameraUrl = url.text.toString().trim()
        ...
        cameras.forEach { camera ->
            ...
            val delete = makeButton("Excluir") {
                ...
                multiViews = multiViews.map { mv ->
                    mv.copy(cameras = mv.cameras.filterNot { it == camera.name })
                }.filter { it.cameras.size == 2 }
                saveCameras(this, cameras)
                saveMultiViews(this, multiViews)
                if (startupTarget !in listOf("HOME") && multiViews.none { it.name == startupTarget }) {
                    startupTarget = "HOME"
        ...
        if (multiViews.isEmpty()) {
            ...
            multiViews.forEach { multiView ->
                ...
                open.layoutParams = LinearLayout.LayoutParams(dp(120), dp(54)).apply {
                ...
                delete.layoutParams = LinearLayout.LayoutParams(dp(130), dp(54)).apply {
                    setMargins(dp(4), dp(4), 0, dp(4))
                }
                row.addView(delete)
                content.addView(row)
        ...
        content.addView(TextView(this).apply {
            text = "Escolha o que será aberto ao iniciar o app:"
            textSize = 16f
            setTextColor(Color.LTGRAY)
        })
        val startupGroup = RadioGroup(this).apply { orientation = RadioGroup.VERTICAL }
        val homeRadio = RadioButton(this).apply {
            id = View.generateViewId()
            text = "Tela principal"
            textSize = 17f
            setTextColor(Color.WHITE)
            isChecked = startupTarget == "HOME"
            tag = "HOME"
        }
        startupGroup.addView(homeRadio)
        multiViews.forEach { multiView ->
            startupGroup.addView(RadioButton(this).apply {
                id = View.generateViewId()
                text = multiView.name
                textSize = 17f
                setTextColor(Color.WHITE)
                tag = multiView.name
                isChecked = startupTarget == multiView.name
            })
        }
    ...
    private fun showMultiViewEditor() {
        ...
        content.addView(TextView(this).apply {
            text = "Selecione exatamente 2 câmeras"
            textSize = 20f
            setTextColor(Color.WHITE)
            setPadding(0, dp(18), 0, dp(6))
        })
        val counter = TextView(this).apply {
            text = "0/2 câmeras selecionadas"
            textSize = 16f
            setTextColor(Color.LTGRAY)
        }
        ...
        cameras.forEach { camera ->
            ...
            check.setOnCheckedChangeListener { button, checked ->
                ...
                if (checked && count > 2) {
                    ...
                    button.setOnCheckedChangeListener { changedButton, changedChecked ->
                        val changedCount = checks.values.count { it.isChecked }
                        if (changedChecked && changedCount > 2) {
                            changedButton.setOnCheckedChangeListener(null)
                            changedButton.isChecked = false
                            changedButton.setOnCheckedChangeListener { _, _ ->
        ...
        if (checks.size < 2) {
            content.addView(message("Adicione pelo menos duas câmeras nas configurações antes de criar uma MultiView."))
        }

        content.addView(makeButton("Salvar MultiView") {
            val selected = checks.entries
                .filter { it.value.isChecked }
                .map { it.key.name }

            val multiViewName = name.text.toString().trim()
            if (multiViewName.isEmpty() || selected.size != 2) {
                return@makeButton
            }

            val multiView = MultiView(multiViewName, selected)
        ...
        buildList {
            for (i in 0 until array.length()) {
                ...
                add(Camera(
                    ...
                    obj.optString("transport", "TCP").uppercase()
                ))
            }
        }
    } catch (_: Exception) {
...
fun saveMultiViews(context: Context, multiViews: List<MultiView>) {
    val array = JSONArray()
    multiViews.forEach { multiView ->
        if (multiView.cameras.size != 2) return@forEach
        val obj = JSONObject()
        obj.put("name", multiView.name)
        obj.put("cameras", JSONArray(multiView.cameras))
        array.put(obj)
    }
    context.getSharedPreferences("multiviews", Context.MODE_PRIVATE)
        .edit().putString("list", array.toString()).apply()
}

fun loadMultiViews(context: Context): List<MultiView> {
    val json = context.getSharedPreferences("multiviews", Context.MODE_PRIVATE)
        .getString("list", null) ?: return emptyList()
    return try {
        val array = JSONArray(json)
        buildList {
            for (i in 0 until array.length()) {
                ...
                val selected = buildList {
                    for (j in 0 until list.length()) add(list.optString(j))
                }
...
fun saveStartupTarget(context: Context, target: String) {
    context.getSharedPreferences("settings", Context.MODE_PRIVATE)
        .edit().putString("startup", target).apply()
}

fun loadStartupTarget(context: Context): String = context
    .getSharedPreferences("settings", Context.MODE_PRIVATE)
    .getString("startup", "HOME") ?: "HOME"
